...
const Cart = () => {
 ...
 const {products, currency, cartItems, removeFromCart, getCartCount, updateCartItem, navigate, getCartAmount, axios,
    setCartItems, user}= useAppContext();
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        ...
                                        <select onChange={e=>updateCartItem(product._id, Number(e.target.value))} value={cartItems[product._id]} className='outline-none'>
                                            {Array(cartItems[product._id]>10 ? cartItems[product._id]>10  : 10 ).fill('').map((_, index) => (
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                               { addresses.map((address,index)=>( <p key={index} onClick={() => { setSelectedAddresses(address);setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                                    {address.street},{address.city},{address.state},{address.country}
                                </p>
                                ))}
                                <p onClick={() => navigate(`/addAddress`)} className="text-primary text-center cursor-pointer p-2 hover:bg-primary-dull/10">
                                    Add address
                                </p>
                            </div>
                        )}
